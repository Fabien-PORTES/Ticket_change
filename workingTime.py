...
def working_from_hours(date):
    ...
    elif date.hour <= 14 and date.hour >= 12:
        delta = pd.Timedelta(hours = 4)
    elif date.hour < 8:
        delta = pd.Timedelta(hours = 8)
    elif date.hour <= 12:
        delta = datetime.datetime(date.year, date.month, date.day, 18, 0, 0) - date - pd.Timedelta(hours = 2)
    else:
...
def working_to_hours(date):
    if date.hour < 8:
        ...
        delta = date - datetime.datetime(date.year, date.month, date.day, 8, 0, 0) - pd.Timedelta(hours = 2)
    else :
...
def working_time(x):
    ...
    working_day = (sum(1 for day in daygenerator if day.weekday() < 5) -1) * 8
    delta_day = pd.Timedelta(0)
    if working_day > 0:
        delta_day = pd.Timedelta(seconds = working_day)
    if delta1 < pd.Timedelta(0):
        print("delta 1 : %s" %delta1, fromdate)
    if delta2 < pd.Timedelta(0):
        print("delta 2 : %s," %delta2, todate )
